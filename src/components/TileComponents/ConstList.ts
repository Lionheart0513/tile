export namespace RSLst {
    ...
    */
	...
	type IOArgs = BlobPack|Function|undefined;

	// Convert DBclass to/from BlobPack
...
export class NameData {
	...
	constructor(Name1: string, DType: string, Buf: ArrayBuffer | undefined = undefined) {
		this.Name = Name1;
		this.DataType = DType;
		this.Buffer = Buf;
	}
...
export class RSData {
		...
		InitPack () {
			let P = new BlobPack ();
			P.Add (['name',this._Name,'_desc',this._Desc,
						'type',this._Type,'!table',this._Tile,
						'_str',this._Str,'!rowid',this._ID]);
		...
		SetTile (TName : string) { this._Tile = TName; }

		get Str () { return this._Str; }

		SetStr (Str1 : string) { this._Str = Str1; }
		...
		ToValue () {
			let ValStr = '';
			if (this.Fmt) {
				if ((this.Fmt.Value)  &&  (this.Fmt.Value.Str))
					ValStr = '=' + this.Fmt.Value.Str;
			}
		...
		ToSelect(Select: SelectArgs) {
			if (Select  && (Select instanceof HTMLSelectElement)) {
				let Option: HTMLOptionElement = Select.ownerDocument.createElement('option') as HTMLOptionElement;
	...
	export function RSDataVert (In : IOArgs) : IOArgs {
		if (In) {
			if (In.constructor.name === 'BlobPack') {
				let Pack = In as BlobPack;
				let Data = new RSData ();
			...
			else {	// must be class for conversion
...
*/
	...
	export class ListOfLists {
		...
		async Defines(FileName: string = 'Consts.ts') {
			...
			let BP = new BlobPack ('TEST','Details : , asdfasdfas');
			let StringBlob = new Blob (['ABCD','0123456789']);
			BP.Add ([ 'Num1', 123, 'Num2', -789.123, 'ShortStr', 'ABC', 'LongStr', '0123456789', 'StringBlob', StringBlob]);
			let NewBlob = BP.BlobOut ();
			await BP.BlobIn (NewBlob);
			console.log ('Resultant Blob:' + '\n' + BP.Desc ());
		}
	...
	export class RsLOL extends ListOfLists {
        ...
        */
		...
		PL = this.Add ('|Number:#|String:$|Blob:%|ArrayBuffer:[|');

		FT = this.Add(
			'Ft|#:Num|I:Int|$:Dollar|P:Pair|O:Ord|A:Nums|%:Str|U:Upper|@:Member|R:Range|{:Set|'
		); // Added & tested full support for Num, Int, Str, Dollar, Nums, Range, Upper, Ord, Pair; Member Rough Support Added
		//
	...
	export class PackField {
		...
		Desc () {
			let Str = 'Name: ' + this.Name + ' Type: ' + this.Type + ' Ch:' + this.TypeCh;
			if ((this.TypeCh === '$')  || (this.TypeCh === '#'))
				Str += ' =' + (this.Data as string);
			else Str += ' ' + this.Size.toString() + ' bytes'			
			return Str;
		}
	}
	...
	export class BlobPack {
		...
		constructor (_Type = '', _Details='') {
			...
			if (BPos > 0)
				_Details = _Details.slice (0,BPos);
		...
		Data (Name : string) : any {
			...
			return F ? F.Data : undefined;
		}

		Str (Name : string) {
			let F = this.Field (Name);
			return (F && F.Data) ? F.Data as string : '';
		}

		Num (Name : string) {
			let F = this.Field (Name);
			return (F && F.Data) ? F.Data as number : 0;
		}

		Desc () {
			let Lines = [];
			let limit = this.Fields.length;
			let Str = 'Blob Type:' + this.Type + ' Details:' + this.Details + '.';
			Lines.push (Str);

			for (let i = 0; i < limit; ++i) {
				Lines.push (this.Fields[i].Desc ());
			}
		...
		Add (Args  : any []) {
			...
			for (let i = 0; i < limit;) {
				...
				let Data = Args[i++];
				if (!Data)
					continue;	// skip this one, no data

				let Replace = this.Field (FldName);
				let NewField = Replace ? Replace : new PackField ();

				NewField.Name = FldName;
				NewField.Data = Data;

				let Type = NewField.Type = NewField.Data.constructor.name;

				if (!(TypeCh = PList.GetDesc (Type))) {
				...
				switch (TypeCh) {
					case '#' : Data = NewField.Data = (Data as Number).toString (); Size = (Data as String).length; break;
					case '%' : Size = (Data as Blob).size; break;
					case '$' : Size = (Data as string).length; break;
					case '[' : Size = (Data as ArrayBuffer).byteLength; break;
					default : return;
				}

				NewField.Size = Size;
				NewField.TypeCh = TypeCh;
				NewField.Type = Type;

				if (!Replace)
					Fields.push (NewField);

				console.log ((Replace ? 'Replacing ' : 'Adding ') + NewField.Desc ());
			}
		}

		private GetPrefix () : string {
			let PFs = this.Fields;
			let limit = PFs.length;
			...
			for (let i = 0; i < limit;) {
				let PF = PFs[i++];
				Prefix += ',' + PF.TypeCh + PF.Name + ':' + PF.Size.toString ();
			}

			let LenStr = Prefix.length.toString ();
			let Str = LenStr + Prefix.slice (LenStr.length);
			if (LenStr.length > 4)
				Str = 'PREFIX TOO LONG!';

			console.log ('GetPrefix = ' + Str + '. Len ' + Str.length);
			return Str;
		}
		...
		BlobOut () : Blob {
			...
			for (let i = 0; i < limit; ++i)
				Args.push (Fields[i].Data);

			let NewBlob = new Blob (Args);

			console.log ('BlobOut Bytes ' + NewBlob.size.toString ());
			return NewBlob;
		}
		...
		async BlobIn (B : Blob) {
			...
			let PBlob = B.slice (0,BufLim);
			let Prefix = await PBlob.text ();
			let Bytes = Number (Prefix.slice (0,4));
			if (Bytes > BufLim) {
				PBlob = B.slice (0,Bytes);
				Prefix = await PBlob.text ();
			}
			...
			Prefix = Prefix.slice (0,Bytes);
			let Offset = Prefix.length;
			console.log ('Import Prefix = ' + Prefix + ' Type =' + this.Type + ' Details=' + this.Details + '. Len=' + Prefix.length.toString ());
			...
			while ((NPos = Prefix.indexOf (',',NPos)) > 0) {
					if ((SPos = Prefix.indexOf (':',++NPos)) > 0) {
						let NewFld = new PackField ();

						NewFld.TypeCh = Prefix[NPos];
						NewFld.Name = Prefix.slice (NPos + 1, SPos);

						NumStr = Prefix.slice (++SPos, SPos + 12);
						if ((EndPos = NumStr.indexOf (',')) >= 0)
							NumStr = NumStr.slice (0,EndPos);

						NewFld.Size = Number (NumStr);

						let FBlob = B.slice (Offset,Offset + NewFld.Size);

						switch (NewFld.TypeCh) {
							case '#' : NewFld.Type = 'Number'; NewFld.Data = await FBlob.text (); NewFld.Data = Number (NewFld.Data); break;
							case '%' : NewFld.Type = 'Blob'; NewFld.Data = FBlob; break;
							case '$' : NewFld.Type = 'String'; NewFld.Data = await FBlob.text ();
										console.log ('Imported String ' + NewFld.Name + '=' + (NewFld.Data as string) + ' Bytes = ' + NewFld.Size.toString ());  break;
							case '[' : NewFld.Type = 'ArrayBuffer'; NewFld.Data = await FBlob.arrayBuffer (); break;
							default : NewFld.TypeCh = ' '; NewFld.Data = undefined;
						}

						this.Fields.push (NewFld);

						console.log ('  Importing ' + NewFld.Desc ());
						Offset += NewFld.Size;
						NPos = SPos;
					}
			}
